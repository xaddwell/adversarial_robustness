...
def generate_datasets(_method,target_class=None,use_cuda = True):
    ...
    for j,item in tqdm(enumerate(dataloader)):
        ...
        for i in range(len(y)):
            if advs_pred[i]!= y[i] and pred[i] == y[i]:
...
if __name__ == "__main__":
    ...
    #加载ImageNet数据集
    dataloader = DataLoader(imageNet_datasets(imageNet_dir,transform=train_transform),
                            generate_data_batch_size,shuffle=True)
    target_class = None
    for target_model_name in tqdm(["ResNet18","ShuffleNetv2","MobileNetv2","DenseNet121"]):
        target_model = get_classifier(target_model_name).cuda()
        target_model.eval()
        for atk_method in tqdm(["BIM","TIFGSM"]):
